...
class EditBusinessModal extends StatefulWidget {
  ...
  const EditBusinessModal({super.key, this.isMandatory = false});

  @override
  State<EditBusinessModal> createState() => _EditBusinessModalState();
}

class _EditBusinessModalState extends State<EditBusinessModal> {
  ...
  void initState() {
    super.initState();
    final business = Provider.of<AuthProvider>(context, listen: false).currentBusiness;
    
    _nameCtrl = TextEditingController(text: business?.commercialName ?? "");
    _rucCtrl = TextEditingController(text: business?.ruc ?? "");
    _addressCtrl = TextEditingController(text: business?.address ?? ""); 
    _paymentCtrl = TextEditingController(text: business?.paymentInfo ?? "");
    ...
    if (business?.printerConfig != null && business!.printerConfig!.isNotEmpty) {
      try {
        ...
        _showAddress = prefs['show_address'] ?? true;
        _showRuc = prefs['show_ruc'] ?? true;
      } catch (e) {
        _showAddress = true;
        _showRuc = true;
      }
  ...
  void _clearLocation() {
    ...
    CustomSnackBar.show(context, message: "Mapa eliminado. Presiona GUARDAR para confirmar.", isError: false);
  }
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(child: Text(widget.isMandatory ? "¡Bienvenido! Registra tu Negocio" : "Datos del Negocio", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black87))),
              ...
              TextFormField(controller: _nameCtrl, style: textStyle, decoration: _deco("Nombre Comercial", Icons.store, isDark), validator: (v) => v!.isEmpty ? "Requerido" : null),
              ...
              const SizedBox(height: 16),
              TextFormField(controller: _addressCtrl, style: textStyle, decoration: _deco("Referencia Escrita (Ej: Frente al parque)", Icons.location_city, isDark)),
              const SizedBox(height: 16),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: auth.isLoading ? const CircularProgressIndicator(color: Colors.white) : Text(widget.isMandatory ? "CREAR NEGOCIO Y EMPEZAR" : "GUARDAR CAMBIOS", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16, letterSpacing: 1)),
                ),
              )
            ],
          ),
